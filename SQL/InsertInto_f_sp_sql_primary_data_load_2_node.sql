...
begin
    ...
    -- LNK_GRBS_CODE_CUSTOMER
    idx := idx + 1;
    rec_array(idx).table_name := 'LNK_GRBS_CODE_CUSTOMER';
    rec_array(idx).sql_name := 'LNK_GRBS_CODE_CUSTOMER';
    rec_array(idx).description := 'Линк ГРБС по коду';
    rec_array(idx).execute_order := idx * 100;
    rec_array(idx).is_actual := 1;
    rec_array(idx).sql_text := start_str || q'#
       insert into reports.LNK_GRBS_CODE_CUSTOMER(ID_CUSTOMER,GRBS_CODE,EAIST) select ID_CUSTOMER,GRBS_CODE,EAIST from LNK_GRBS_CODE_CUSTOMER@reports1;
    ...
    rec_array(idx).sql_text := start_str || q'#
        INSERT INTO REPORTS.SP_KOSGU (ID,
            ...
            SELECT ID,
                   ...
                   V_VERSION_DATE,
                   ENTITY_ID
              FROM REPORTS.SP_KOSGU@reports1
             WHERE ID_DATA_SOURCE = V_ID_DATA_SOURCE
    ...
    rec_array(idx).sql_text := start_str || q'#
        INSERT INTO REPORTS.SP_SECTION_BUDGET (ID,
            ...
            SELECT ID,
                   ...
                   V_ID_DATA_SOURCE,
                   V_VERSION_DATE
              FROM SP_SECTION_BUDGET@reports1
             WHERE ID_DATA_SOURCE = V_ID_DATA_SOURCE
                   AND VERSION_DATE = (SELECT MAX (VERSION_DATE)
                                         FROM REPORTS.SP_SECTION_BUDGET@reports1
                                        WHERE ID_DATA_SOURCE = V_ID_DATA_SOURCE);
    ...
    rec_array(idx).sql_text := start_str || q'#
        INSERT INTO REPORTS.SP_TARGET_CLAUSE (ID,
            ...
            SELECT ID,
                   ...
                   V_ID_DATA_SOURCE,
                   V_VERSION_DATE
              FROM SP_TARGET_CLAUSE@reports1
             WHERE ID_DATA_SOURCE = V_ID_DATA_SOURCE
                   AND VERSION_DATE = (SELECT MAX (VERSION_DATE)
                                         FROM REPORTS.SP_TARGET_CLAUSE@reports1
                                        WHERE ID_DATA_SOURCE = V_ID_DATA_SOURCE);
    ...
    rec_array(idx).sql_text := start_str || q'#
        INSERT INTO REPORTS.SP_TYPE_EXPENSE (ID,
            ...
            SELECT ID,
                   ...
                   V_ID_DATA_SOURCE,
                   V_VERSION_DATE
              FROM SP_TYPE_EXPENSE@reports1
             WHERE ID_DATA_SOURCE = V_ID_DATA_SOURCE
                   AND VERSION_DATE = (SELECT MAX (VERSION_DATE)
                                         FROM REPORTS.SP_TYPE_EXPENSE@reports1
                                        WHERE ID_DATA_SOURCE = V_ID_DATA_SOURCE);
...
SELECT pv.ID as Id1,
                           ...
                           dates.first_pub_date, --FIRST_PUBLSIH_DATE
                           dates.last_pub_date, --LAST_PUBLISH_DATE
						   pe.published_id --PREV_ENTITY_ID
                    ...
                    LEFT JOIN (select distinct ent.reg_number,
                                min(case when ver.status_id=7 then ver.status_date else null end) over (partition by ent.reg_number) first_pub_date,
                                max(case when ver.status_id=7 then ver.status_date else null end) over (partition by ent.reg_number) last_pub_date
                                FROM d_procedure_status_history@eaist_mos_shard ver
                                JOIN D_PROCEDURE_ENTITY@EAIST_MOS_SHARD ent
                                ON ver.entity_id=ent.id) dates on dates.reg_number=pe.reg_number
    ...
    rec_array(idx).sql_text := start_str || q'#
        INSERT INTO REPORTS.T_PURCHASE_PLAN (ID,
            ...
            SELECT dppv.ID,
                   ...
                   pph.status_date,
                   dppv.version,
                   dates1.first_pub_Date,
                   dates.last_change_pub_date,
                   dates1.last_pub_date,
                   dates1.first_approve_date,
                   dates.last_change_approve_date,
                   dates1.last_approve_date
              FROM D_PURCHASE_PLAN_ENTITY@EAIST_MOS_SHARD dppe
                   ...
                   JOIN (  SELECT YEAR,
                    ...
                    LEFT JOIN (select * from (                
                                ...
                                ON ver.id=stat.version_id
                                ) where coalesce(last_change_pub_date,last_change_approve_date) is not null) dates on dates.entity_id=dppe.id
                    LEFT JOIN (select * from (                
                                select distinct pe.customer_id, pe.period,
                                min(case when stat.status_id=8 then status_date else null end) over (partition by pe.customer_id, pe.period) first_pub_date,
                                max(case when stat.status_id=8 then status_date else null end) over (partition by pe.customer_id, pe.period) last_pub_date,
                                min(case when stat.status_id=5 then status_date else null end) over (partition by pe.customer_id, pe.period) first_approve_date,
                                max(case when stat.status_id=5 then status_date else null end) over (partition by pe.customer_id, pe.period) last_approve_date
                                FROM d_purchase_plan_entity@eaist_mos_shard pe
                                JOIN D_PURCHASE_PLAN_STATUS_HISTORY@EAIST_MOS_SHARD stat
                                ON pe.id=stat.entity_id) where coalesce(first_pub_date,first_approve_date) is not null) dates1 on dates1.customer_id=dppe.customer_id and dates1.period=dppe.period;
...
END;#';
    ...
    rec_array(idx).is_actual := 1;
    rec_array(idx).sql_text := start_str || q'#
        INSERT INTO T_LOT_NMC_EXAMINATION (ID, LOT_ID, SEND_DATE, APPROVED_DATE, EXPECTED_EXAMINATION_DATE, CURRENT_STATE, EXPECTED_EXAMINATION_END_DATE, ID_DATA_SOURCE, version_date, GRBS, REG_NUMBER, NMC, EXAMINATION_ORG_ID)
		SELECT ex.ID, ex.LOT_ID, ex.SEND_DATE, ex.APPROVED_DATE, ex.EXPECTED_EXAMINATION_DATE, ex.CURRENT_STATE, ex.EXPECTED_EXAMINATION_END_DATE, V_ID_DATA_SOURCE ID_DATA_SOURCE, V_VERSION_DATE version_date, grbs_code, REG_NUMBER, NMC, EXAMINATION_ORG_ID 
		FROM (SELECT ID, LOT_ID, SEND_DATE, APPROVED_DATE, EXPECTED_EXAMINATION_DATE, CURRENT_STATE, EXPECTED_EXAMINATION_END_DATE, REG_NUMBER, NMC, EXAMINATION_ORG_ID FROM D_LOT_NMC_EXAMINATION@eaist_mos_shard) ex
...
END;#';
    ...
    rec_array(idx).sql_text := start_str || q'#
       ...
       SELECT ID, EAIST1_ID, EAIST2_ID, V_VERSION_DATE from LNK_METHOD@reports1
		where version_date=(SELECT MAX (VERSION_DATE) FROM REPORTS.LNK_METHOD@reports1);
...
END;#';
    ...
    rec_array(idx).sql_text := start_str || q'#
       ...
       SELECT ID, EAIST1_ID, EAIST2_ID, NAME, V_VERSION_DATE from LNK_STATUS_CATEGORY@reports1
		where version_date=(SELECT MAX (VERSION_DATE) FROM REPORTS.LNK_STATUS_CATEGORY@reports1);
...
END;#';
    ...
    rec_array(idx).sql_text := start_str || q'#
       ...
       SELECT ID, EAIST1_ID, EAIST2_ID, PP_ID, STATUS_CATEGORY_ID, NAME, V_VERSION_DATE from LNK_STATUS@reports1
		where version_date=(SELECT MAX (VERSION_DATE) FROM REPORTS.LNK_STATUS@reports1);
...
END;#';
    ...
    rec_array(idx).sql_text := start_str || q'#
       ...
       SELECT ID, EAIST1_ID, EAIST2_ID, EAIST4_ID, V_VERSION_DATE from LNK_SOURCE_FINANCE@reports1
		where version_date=(SELECT MAX (VERSION_DATE) FROM REPORTS.LNK_SOURCE_FINANCE@reports1);
...
-- SP_TRADING_PLATFORM [EAIST1]
	...
	V_ID_DATA_SOURCE,
	id
	from SP_TRADING_PLATFORM@reports1 
		where version_date=(SELECT MAX(VERSION_DATE) FROM SP_TRADING_PLATFORM@reports1
								WHERE ID_DATA_SOURCE=V_ID_DATA_SOURCE) and ID_DATA_SOURCE=V_ID_DATA_SOURCE;
...
-- LNK_CUSTOMERS_E1_E2_IMP
    idx := idx + 1;
    rec_array(idx).table_name := 'LNK_CUSTOMERS_E1_E2_IMP';
